...
export default function Agenda() {
    ...
    useEffect(() => {
        const atualizarDepoisDoAgendamento = () =>
            carregarAgenda(dataSelecionada);
        window.addEventListener(
            "flooty:agenda-atualizada",
            atualizarDepoisDoAgendamento,
        );
        return () =>
    ...
    const handleSelectPet = async (pet, clientePai) => {
        setPetSelecionado(pet);
        setClienteSelecionado(clientePai);
        setStep(2); // Avança direto para a escolha de serviços

        if (!pet.raca_id) return;
        try {
            const response = await fetch(
                `/api/servicos?raca_id=${pet.raca_id}`,
            );
            if (response.ok) setListaServicos(await response.json());
        } catch (error) {
            toast.error("Não foi possível carregar os valores desta raça.");
        }
    };

    const handleToggleServico = (servico) => {
        const jaExiste = servicosSelecionados.find(
            (s) => s.servico_id === servico.id,
        );
        if (jaExiste) {
            setServicosSelecionados(
                servicosSelecionados.filter((s) => s.servico_id !== servico.id),
            );
        } else {
            setServicosSelecionados([
    ...
    return (
        <div className="agenda-wrapper">
            ...
            <div className="agenda-grid">
                {agendamentos.length === 0 ? (
                    ...
                    agendamentos.map((item) => (
                        <button
                            className="tecpet-card"
                            key={item.id}
                            type="button"
                            onClick={() => setAgendamentoDetalhes(item)}
                        >
                                ...
                                <div className="card-main-info">
                                    ...
                                    <div className="card-footer-icons">
                                        <div className="status-icons">
                                            <span
                                                className={`payment-badge payment-${(item.status_pagamento || "Pendente").toLowerCase()}`}
                                            >
                                            ...
                                            {item.taxi_dog ? (
                                                <span
                                                    className="material-symbols-rounded card-feature-icon"
                                                    title="Taxi Dog"
                                                >
                                                    directions_car
                                                </span>
            ...
            {agendamentoDetalhes && (
                <AgendamentoDetalhes
                    item={agendamentoDetalhes}
                    onClose={() => setAgendamentoDetalhes(null)}
                    onUpdated={(atualizado) => {
                        setAgendamentoDetalhes(atualizado);
